					...
					for group in groupList:
						if group.name == currentGroup:
							...
							stdStuff.objListToFile(groupList, stdStuff.directory,
													stdStuff.groupFile,)
				if form.getvalue('done') == "Add Admin":
					for group in groupList:
						if group.name == currentGroup:
							group.addAdmin(form.getvalue('addAdmin'))
							stdStuff.objListToFile(groupList, stdStuff.directory,
													stdStuff.groupFile,)
					...
					for group in groupList:
						if group.name == currentGroup:
							if groupName in groupDict.keys():
								groupStatus = "Group name is already taken. Group name has not been changed.<br>"
							else:
