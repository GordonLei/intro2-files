...
'''
		...
		if len(form) > 0:
			...
			if found:
				#Add Friend option
				body+= '''<a href = 'addFriend.py?userTarget=''' + searchUser + '''&search=Find+User'>Add Friend</a><br>'''
				bodyCount = True
			...
			if bodyCount and found:
				body+= "User has not posted anything yet!"
			elif not(found):
				body+= "User is not found."
		if len(form) == 0:
                        body += '<h1>Searched Name: NO VALUE INPUTTED</h1>'
                        body += "You did not search a name."
